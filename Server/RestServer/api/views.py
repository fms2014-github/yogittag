...
from RestServer.settings import SECRET_KEY
...
def user_login(request):
    ...
    try:
        ...
        payload = {"aud": email.email, "sub": "login", "iat": time.time().__int__(), "exp": (time.time().__int__() + 50)}
...
def session_refresh(request):
    ...
    if hashlib.sha256(get_jwt[0].encode() +
        ...
        if int(payload['exp']) >= int(time.time().__int__()) :
            ...
            get_jwt[2] = base64.b64encode(hashlib.sha256(get_jwt[0].encode() + get_jwt[1].encode() + SECRET_KEY.encode()).hexdigest().encode()).decode("UTF-8")
