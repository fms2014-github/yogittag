...
def recommand_based_user(request, id=None):
    ...
    user_review = queryset.filter(user_id=id)  # 유저가 리뷰한 것
    print("유저의 해당지역 리뷰 개수 : {}".format(len(user_review)))
    ...
    if len(user_review) == 0:
        ...
        semires = queryset.filter(user_id__in=other_user).values(
    ...
    # 리뷰개수가 5개 미만인 것은 필터링
    count_df = dataframe.groupby(['store_id']).size().reset_index().rename(
        columns={0: "count"})

    count_df = count_df[count_df['count'] >= 5]

    processed_data = pd.merge(
        count_df, dataframe, left_on='store_id', right_on='store_id') """
